...
#=
    ...
    A case script includes scaleTrack.jl and calls one of the two drivers:

    - init_async_tracking!(executor; ...): the production coupling.  The
      tracking runs asynchronously with respect to the OpenFOAM time loop
      (see asyncTracking.jl); every MPI rank participates.

    - init_sync_tracking!(executor; ...): a synchronous, single-rank variant
      in which evolve_cloud blocks until the tracking of the time step is
      complete.  Used for regression testing (see
      test/icoJuliaParcelFoam/cavity3D/testThreadedTracking.jl).
    ...
    internal storage to it (juliaWrapper::reallocate on the C++ side).
=#
...
function evolve_cloud(Δt, ::SyncMode)
    ...
    sync_evolve!(chunk, reg["eulerian"], mesh, Δt, executor)
...
#=
    Set up the asynchronous two-way coupled tracking.
    ...
    diameters (init! in particles.jl).
=#
function init_async_tracking!(
    ...
    @show nParticles nChunks μᶜ ρᶜ ρᵈ nCellsPerDirection origin ending
    @show decomposition gcTimeStepInterval
    ...
    # shared with OpenFOAM by pointer (allocate_array_j)
    ...
    if comm.isHost
        ...
        for i in eachindex(chunks)
            chunks[i] =
                allocate_chunk(comm, executor, nParticles, μᶜ, ρᵈ, ρᵈ/ρᶜ)
        ...
        for i in eachindex(reg["eulerian"])
            reg["eulerian"][i] =
                TwoWayEulerian{VectorField}(prod(mesh.partitionN))
        end
...
function init_sync_tracking!(
    ...
    @show nParticles μᶜ ρᶜ ρᵈ nCellsPerDirection origin ending
    ...
    global chunk = allocate_chunk(executor, nParticles, μᶜ, ρᵈ, ρᵈ/ρᶜ)
    ...
    reg["eulerian"] = TwoWayEulerian{VectorField}(Int(prod(mesh.N)))
...
# Fill the carrier velocity fields with seeded random data
function randomize_velocity!()
    e = reg["eulerian"]
    if e isa TwoWayEulerian
