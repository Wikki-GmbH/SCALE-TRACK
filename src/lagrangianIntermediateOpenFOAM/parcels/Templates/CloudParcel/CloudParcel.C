...
//- Water vapour saturation pressure (Arden-Buck equation) in Pa, for a
//  temperature in K
inline Foam::scalar saturationPressure(const Foam::scalar T)
{
    const Foam::scalar TdegC = T - 273.15;
    return 611.21*Foam::exp((18.678 - TdegC/234.5)*(TdegC/(257.14 + TdegC)));
}
...
) const
{
    ...
    // Thermal conductivity of air from a linear fit in the carrier
    // temperature, rather than the interpolated carrier field
...
)
{
    ...
    // Mass Transfer
    // Water vapour saturation pressure (Arden-Buck-Equation) in Pa, of the
    // carrier and at the droplet surface.  The two are at different
    // temperatures, and the saturation is steeply temperature dependent
    const scalar pvsatc = saturationPressure(td.Tc());
    const scalar pvsatd = saturationPressure(this->T_);

    // water vapour density at saturation in kg/m³
    // 18.01528e-3 -> molar mass of water
    const scalar rhovsatc =
        18.01528e-3*pvsatc/(physicoChemical::R.value()*td.Tc());
    const scalar rhovsatd =
        18.01528e-3*pvsatd/(physicoChemical::R.value()*this->T_);

    //air molecular density (43.04*Tref*p/(T*pref)) in mol/m³
    const scalar rhoMolAir = 43.04*283.15/(td.Tc()*1.01325);

    // Water vapour pressure in Pa, at an assumed ambient pressure of 1e5 Pa
    const scalar pv = td.rhoVc()/(rhoMolAir*18.01528e-3)*1.0e5;

    // Saturation ratio of water vapour in continuous phase
    const scalar Sinf = pv/pvsatc;
    ...
    // Integration over time (semi-implicit Euler), the driving force being the
    // vapour density of the carrier less the one at the surface
    const scalar bcp = 2.0*3.14159*cloud.constProps().DiffCo()*this->d_;

    const scalar dm = bcp*(rhovsatc*Sinf - rhovsatd*Ssfc)*dt;
    // Floored at the mass of a droplet of about 1 um
    const scalar mNew = max(5.0e-16, mass0 + dm);
    drhoVTrans -= mNew - mass0;

    // latent heat release
    this->T_ -=
        drhoVTrans*cloud.constProps().SLH()/(this->Cp_*0.5*(mNew + mass0));
