...
# Case parameters of singleCellEvaporation.  The library must be included
# before this file.
#
# One droplet in one cell, evaporating into carrier air that starts at the
# droplet temperature.  Convective heat transfer is off, so the droplet cools
# only by the latent heat it carries away and hands the carrier no thermal
# energy; the vapour mass is the only source.  Allrun runs the OpenFOAM
# reference solver on the same case and greps both for comparison.
