...
function init!(chunk, mesh, executor, randSeed=19891)
    ...
    fill!(c.d, 500e-6SCL)
    fill!(c.T, 293.15SCL)   # @SiSc: added temperature
    fill!(c.U, 0.0)
    fill!(c.V, 0.0)
    fill!(c.W, 0.0)
    return nothing
...
# @SiSc: set_parcel_state

# @SiSc: included temperature and diameter, removed state0
@inline function update!(
    eulerian::TwoWayEulerian, dUTrans, dhTrans, drhoVTrans, posI, ::CPU
)
    # momentum transfer (only for coupled part of the forces)
    @inbounds eulerian.UTrans[posI] += dUTrans

    # mass transfer
    @inbounds eulerian.rhoVTrans[posI] += drhoVTrans

    # thermal energy transfer (temperature change and latent heat release)
...
#        @inbounds extrapolator.prevRhoVTrans[i] = currHTrans[i]
#    end
end
...
function init_async_evolve!(eulerian, control, comm::Comm{Slave}, ::GPU)
    ...
    while true
        ...
        # @SiSc: changed to eulerianSreqsU; added T and U
        ...
        # @SiSc: changed to eulerianSreqsU; added T and U
        # question @Sergey: are additional waits for U and T necessary?
...
@inline function evolve_particle!(
    ...
    @inbounds begin
        ...
        for t = 1LBL:nSteps
            ...
            mᵈNew = max(
                5e-16SCL,scalar(mᵈ + 2SCL*π*Dᵈᶜ*⌀*rhovsat*(Sinf - Ssfc)*Δt))
            Δmᵈ = mᵈNew - mᵈ
            ...
            uᵈNew   = uᵈ .+ Δuᵈ
            uᵈNewCp = uᵈ .+ ΔuᵈCp
            dUTrans.x -= 0SCL # mᵈNew*uᵈNewCp.x - mᵈ*uᵈ.x 
            dUTrans.y -= 0SCL # mᵈNew*uᵈNewCp.y - mᵈ*uᵈ.y
            dUTrans.z -= 0SCL # mᵈNew*uᵈNewCp.z - mᵈ*uᵈ.z
        ...
        linearMomentumZ = uᵈ.z*mᵈ
        @cuprint("momentum.z = %.3e \n", linearMomentumZ)
    end
    return nothing
end
...
function evolve_cloud(Δt)
    ...
    GC.enable(false)
    minHTrans = minimum(reg["eulerian"][comm.jlRank].hTrans)
    maxHTrans = maximum(reg["eulerian"][comm.jlRank].hTrans)
    @debugCommPrintln("min/max hTrans: $minHTrans; $maxHTrans")
