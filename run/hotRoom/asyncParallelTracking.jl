...
@inline function evolve_particle!(
    ...
    @inbounds begin
        ...
        for t = 1LBL:nSteps
            ...
            TᵈNew   = Tᵈ + ΔTᵈ
            dhTrans += Cₚᵈ*(mᵈ*Tᵈ - mᵈNew*TᵈNew)

            ### velocity ###

            # drag force with Schiller-Naumann drag coefficient model
            Cd = scalar(24SCL*(1SCL + (Re^0.687SCL)*0.15SCL))
            Fd = scalar(mᵈ*0.75SCL*μᶜ*Cd*Re/(ρᶜ*⌀^2SCL))
            ...
            uᵈNew   = uᵈ .+ Δuᵈ
            uᵈNewCp = uᵈ .+ ΔuᵈCp
            dUTrans.x += mᵈ*uᵈ.x - mᵈNew*uᵈNewCp.x
            dUTrans.y += mᵈ*uᵈ.y - mᵈNew*uᵈNewCp.y
            dUTrans.z += mᵈ*uᵈ.z - mᵈNew*uᵈNewCp.z
...
nParticles = 1_000_000  # RTX 6000 Ada
